...
class HBTReader:
    ...
    def __init__(self, subhalo_path):
        ...
        # useful in case there are missing snapshots in the directory
        self._valid_snapshots = None

    @property
    def valid_snapshots(self):
        if self._valid_snapshots is None:
    ...
    def LoadParticles(self, isnap=-1, subindex=None, filetype='Sub'):
        ...
        offset=0
        for i in range(max(self.nfiles,1)):
            with h5py.File(self.GetFileName(isnap,  i, filetype), 'r') \
                    as subfile:
                if subindex is None:
            ...
            with h5py.File(self.GetFileName(isnap,  i), 'r') as subfile:
                ...
                if offset+nsub>subindex:
                    ...
                    try:
                        ...
                        field = f'ParticleProperties/Sub{subindex-offset}'
                        return subfile[field][...]
                    except:
    ...
    def GetTrack(self, trackId, fields=None):
        ...
        # to account for missing snapshots. Use the last snapshot
        # to get the fields we need
        test_tracksnap = self.GetTrackSnapshot(trackId, -1)
        for i, is_valid in enumerate(self.valid_snapshots):
            if is_valid:
                track.append(
                    self.GetTrackSnapshot(trackId, snaps[i], fields=fields))
                scales[i] = self.GetScaleFactor(snaps[i])
            else:
                track.append(np.zeros_like(test_tracksnap))
        track = append_fields(
            np.array(track), ['Snapshot', 'ScaleFactor'], [snaps, scales],
            usemask=False)
        return track[self.valid_snapshots]

    def GetScaleFactor(self, isnap):
        try:
            scale = h5py.File(
                self.GetFileName(isnap),'r')['Cosmology/ScaleFactor'][0]
        except:
            scale = h5py.File(self.GetFileName(isnap),'r')['ScaleFactor'][0]
        return scale
