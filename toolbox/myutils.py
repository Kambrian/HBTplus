...
def create31fig(sharex=True, sharey=False, figsize=(8, 8)):
    ...
    plt.setp([a.yaxis for a in ax[:-1]],
             major_locator=MaxNLocator(nbins=nbins, prune='lower', symmetric=True))
    return ax
...
def lnADPDF(lnAD):
    # w, mu, sigma, bi-normal fit
    gpar = [[0.569,   -0.570,    0.511], [0.431,    0.227,    0.569]]
    p = gpar[0][0]*norm.pdf(lnAD, gpar[0][1], gpar[0][2]) + \
        gpar[1][0]*norm.pdf(lnAD, gpar[1][1], gpar[1][2])
    return p


def ADSurvFunc(AD):
    '''survival function for AD test on uniform distributions'''
    gpar = [[0.569,   -0.570,    0.511],
            [0.431,    0.227,    0.569]]  # w, mu, sigma, bi-normal fit
    lnAD = np.log(AD)
    p = gpar[0][0]*norm.sf(lnAD, gpar[0][1], gpar[0][2]) + \
        gpar[1][0]*norm.sf(lnAD, gpar[1][1], gpar[1][2])
    return p
...
class ProgressMonitor:
    ...
    def monitor_progress(self, current_step):
        ...
        if current_step >= self.total_steps*self.current_show/self.total_show:
            print("\b\b\b\b\b %02d%%" %
                  (self.current_show*100/self.total_show), end=' ')
            sys.stdout.flush()
            self.current_show += 1
...
def percentile_contour(X, Y, Z, percents=0.683, colors=None, fill=False, linestyles='solid', **kwargs):
    ...
    if fill:
        h0 = plt.contourf(X, Y, Z, lvls, colors=colors,
                          linestyles=linestyles, **kwargs)
    else:
        h0 = plt.contour(X, Y, Z, lvls, colors=colors,
                         linestyles=linestyles, **kwargs)
    ...
    try:
        ...
        color = colors
    h = Ellipse((0, 0), 0, 0, fill=fill, color=color,
...
def get_extent(X, Y):
    ...
    if X.squeeze().ndim == 2:
        ...
        dx = X[1]-X[0]
        dy = Y[1]-Y[0]
    extent = (X.ravel().min()-dx/2, X.ravel().max()+dx/2,
              Y.ravel().min()-dy/2, Y.ravel().max()+dy/2)
    return extent
...
def plot_cov_ellipse(cov, pos, nstd=1, fill=False, ax=None, **kwargs):
    ...
    # the TS=x'* at the specific nstd, for a 2-d
    TS = {1: 2.3, 2: 6.18, 3: 11.8}
...
def skeleton(x, y, nbin=10, alpha=0.683, weights=None):
    ...
    for i in range(nbin):
        if weights is not None:
            xm[i] = np.sum(x[bin == i]*weights[bin == i]) / \
                np.sum(weights[bin == i])
            ym[i] = np.sum(y[bin == i]*weights[bin == i]) / \
                np.sum(weights[bin == i])
